...
export const getTransformFromCss = (transformCssString: string): Coordinator => {
...
export const getNumberedArray = (arr: number[]): NumberedObj[] =>
  arr.map((value: number, number: number) => ({ value, number }));

export const toStringArray = (arr: NumberedObj[]): string[] =>
  arr.map(({ value, number }: NumberedObj) => `${value}_${number}`);
